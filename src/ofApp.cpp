...
void ofApp::setup() {
	...
	spark.loadImage("images/bigstar.jpg");
	...
	if (defaultImage.loadImage("images/emit2.png")) {
		invaders->setChildImage(defaultImage);
		imageLoaded = true;
	}
	else imageLoaded = false;

	if (defaultImage.loadImage("images/emit3.png")) {
	...
	cam.setFov(120);
	//ofSetVerticalSync(true);
	//cam.setTarget(ofVec3f(ofGetWindowWidth() / 2, ofGetWindowHeight() / 2));
	ofSetFrameRate(60);
...
void ofApp::checkCollisions() {
	...
	for (int i = 0; i < playerBullets.size(); i++) {
		...
		//increase difficulty
		invaders->velocity.set(0, invaderVelo +10, 0);
	}
	
	//If they hit blue enemies they get double point
...
void ofApp::update(){
	...
	if (gameState) {
		...
		//	explosive1.setVelocity(velocity);
		...
		//	explosive2.setVelocity(velocity);
	...
	// So 'spriteHurt' is an int , and it increases as the sprite gets attacked by the invaders. So if spriteHurt =5 , game over
	
	/*if (spriteHurt ==10)
	{
		sprite.image.resize(20, 20);
	}*/
...
void ofApp::draw(){
	...
	//how many times is our sprite getting hurt , when spriteHurt=5 -> game over
	string spritehurt;
	spritehurt += "SPRITE: " + std::to_string(spriteHurt);
			...
			*/
			if (shotsFired != 0)
			{
				...
				if (1000 * (clock() - finalShot) / CLOCKS_PER_SEC >= 1000 / shotRate)
				{
					Bullet b;
					b = Bullet();
					b.image.loadImage("images/boom.png");
